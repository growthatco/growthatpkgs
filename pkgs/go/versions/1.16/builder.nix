...
, runtimeShell, buildPackages, pkgsBuildTarget, fetchpatch, callPackage }:

{ name, version, parent, packageFile, sha256 }@args:

let
  inherit (builtins) toString;

  go_bootstrap = buildPackages.callPackage (packageFile "bootstrap.nix") { };
...
in stdenv.mkDerivation rec {
  ...
  patches = [
    (packageFile "remove-tools-1.11.patch")
    (packageFile "ssl-cert-file-1.16.patch")
    (packageFile "remove-test-pie-1.15.patch")
    (packageFile "creds-test.patch")
    (packageFile "go-1.9-skip-flaky-19608.patch")
    (packageFile "go-1.9-skip-flaky-20072.patch")
    (packageFile "skip-external-network-tests-1.16.patch")
    (packageFile "skip-nohup-tests.patch")
    (packageFile "skip-cgo-tests-1.15.patch")
    (packageFile "go_no_vendor_checks-1.16.patch")
  ] ++ [
    # breaks under load: https://github.com/golang/go/issues/25628
    (if stdenv.isAarch32
    then (packageFile "skip-test-extra-files-on-aarch32-1.14.patch")
    else (packageFile "skip-test-extra-files-on-386-1.14.patch"))
  ];
