...
, stripBytecode ? reproducibleBuild, includeSiteCustomize ? true
...
stdenv.mkDerivation {
  ...
  patches = [
    ...
    (nixpkgs_file "cpython/${sourceVersion.major}.${sourceVersion.minor}/no-ldconfig.patch")
  ] ++ optionals mimetypesSupport [
    # Make the mimetypes module refer to the right file
    (nixpkgs_file "cpython/mimetypes.patch")
  ] ++ optionals (isPy35 || isPy36) [
    # Determinism: Write null timestamps when compiling python files.
    (nixpkgs_file "cpython/3.5/force_bytecode_determinism.patch")
  ] ++ optionals isPy35 [
    # Backports support for LD_LIBRARY_PATH from 3.6
    (nixpkgs_file "cpython/3.5/ld_library_path.patch")
    ...
    (nixpkgs_file "cpython/3.7/fix-hardcoded-path-checking-for-rpmbuild.patch")
    ...
    (nixpkgs_file "cpython/3.7/no-win64-workaround.patch")
  ] ++ optionals (pythonAtLeast "3.7") [
    # Fix darwin build https://bugs.python.org/issue34027
    (nixpkgs_file "cpython/3.7/darwin-libutil.patch")
  ] ++ optionals (pythonOlder "3.8") [
    # Backport from CPython 3.8 of a good list of tests to run for PGO.
    (if isPy36 || isPy37 then
      (nixpkgs_file "cpython/3.6/profile-task.patch")
    else
      (nixpkgs_file "cpython/3.5/profile-task.patch"))
    ...
    (if isPy35 then
      (nixpkgs_file "cpython/3.5/python-3.x-distutils-C++.patch")
    else if pythonAtLeast "3.7" then
      (nixpkgs_file "cpython/3.7/python-3.x-distutils-C++.patch")
    else
      fetchpatch {
        url =
    ...
    (nixpkgs_file
      "cpython/3.8/0001-On-all-posix-systems-not-just-Darwin-set-LDSHARED-if.patch")
    # Use sysconfigdata to find headers. Fixes cross-compilation of extension modules.
    (if isPy36 then
      (nixpkgs_file
        "cpython/3.6/fix-finding-headers-when-cross-compiling.patch")
    else
      (nixpkgs_file
        "cpython/3.7/fix-finding-headers-when-cross-compiling.patch"))
  ] ++ optionals (isPy36) [
    # Backport a fix for ctypes.util.find_library.
    (nixpkgs_file "cpython/3.6/find_library.patch")
  ];
  ...
  NIX_LDFLAGS =
    optionalString (stdenv.isLinux && !stdenv.hostPlatform.isMusl) "-lgcc_s"
    + optionalString stdenv.hostPlatform.isMusl "-lgcc_eh";
